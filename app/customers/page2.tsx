...
// Initial icon data with their websites and 3D positions
const initialIconData: IconData[] = [
  {
    name: "TechCorp",
...
// Arrays for generating fake company names
const prefixes = ["Tech", "Health", "Edu", "Shop", "Finance", "Travel", "Food", "Fit", "Eco", "Smart", "Net", "Cloud"];
const suffixes = ["Corp", "Plus", "Learn", "Mart", "Hub", "Go", "App", "Tracker", "Solutions", "Labs", "World", "Net"];

// Function to generate new icons
function generateNewIcons(count: number, startZ: number): IconData[] {
  const newIcons: IconData[] = [];
  for (let i = 0; i < count; i++) {
    const prefix = prefixes[Math.floor(Math.random() * prefixes.length)];
    const suffix = suffixes[Math.floor(Math.random() * suffixes.length)];
    const name = prefix + suffix;
    const website = `https://${name.toLowerCase()}.example.com`;
    const position: [number, number, number] = [
      Math.random() * 20 - 10, // Wider x spread for variety
      Math.random() * 20 - 10, // Wider y spread
      startZ - Math.random() * 30 - 10, // Spread out in deeper -z
    ];
    const color = Math.random() > 0.5 ? "#01ADEF" : "#ffffff";
    const Icon = prefix[0] + suffix[0];
    newIcons.push({ name, website, position, color, Icon });
  }
  return newIcons;
}
...
function Scene() {
  const [icons, setIcons] = useState<IconData[]>(initialIconData);
  ...
  useEffect(() => {
    const handleChange = () => {
      ...
      const targetZ = controlsRef.current.target.z;
      const currentMinZ = Math.min(...icons.map((i) => i.position[2]));

      // Trigger adding more icons when the orbit target approaches the current deepest z (with a buffer)
      if (targetZ < currentMinZ + 20) {
        const newStartZ = currentMinZ - 20;
        const newIcons = generateNewIcons(8, newStartZ);
